...
LOG_PATH = STATE_DIR / "automarimo.log"
LOG_OLD_PATH = STATE_DIR / "automarimo.old.log"
...
def choose_from_list_macos(title: str, prompt: str, options: list[str]) -> str | None:
    if not options:
        return None

    items = ", ".join(f'"{applescript_quote(x)}"' for x in options)
    script = f'''
set theChoice to choose from list {{{items}}} with title "{applescript_quote(title)}" with prompt "{applescript_quote(prompt)}" default items {{"{applescript_quote(options[0])}"}} OK button name "Choose" cancel button name "Cancel"
...
def choose_application_macos() -> list[str] | None:
    script = r'''
set chosenApp to choose file of type {"com.apple.application-bundle"} with prompt "Choose your code editor application"
...
'''.strip()
    ...
    if completed.returncode != 0:
        ...
        if "user canceled" in stderr or "(-128)" in stderr:
            return None
        raise UserFacingError(
            f"macOS application chooser failed.\nstderr:\n{completed.stderr or '(none)'}"
        )
    ...
    if selected_path.suffix.lower() != ".app":
        raise UserFacingError(
            f"Selected application is not a .app bundle:\n  {selected_path}"
        )
...
# def prompt_for_editor_gui_macos(cfg: Config, candidates: list[list[str]]) -> list[str] | None:
#     try:
#         import tkinter as tk
#     except Exception:
#         return None
...
#     def choose_command(cmd: list[str]) -> None:
#         nonlocal result
#         result = cmd
#         root.destroy()

#     def choose_browse() -> None:
#         nonlocal result
#         cmd = prompt_for_editor_path_macos()
#         if cmd is not None:
#             result = cmd
#         root.destroy()
...
#         nonlocal result
#         result = ["__DEFAULT_APP__"]
#         root.destroy()

#     root = tk.Tk()
#     root.title("automarimo setup")
#     root.attributes("-topmost", True)

#     label = tk.Label(root, text="Choose the editor to use for normal Python files.")
#     label.pack(padx=16, pady=(16, 8))

#     for candidate in candidates:
#         text = " ".join(candidate)
#         btn = tk.Button(root, text=text, anchor="w", command=lambda c=candidate: choose_command(c))
#         btn.pack(fill="x", padx=16, pady=4)

#     browse_btn = tk.Button(root, text="Browse for an editor application...", anchor="w", command=choose_browse)
#     browse_btn.pack(fill="x", padx=16, pady=(12, 4))

#     default_btn = tk.Button(root, text="Use the current macOS default app for normal .py files", anchor="w", command=choose_default)
#     default_btn.pack(fill="x", padx=16, pady=(4, 16))
...
def prompt_for_editor_macos(cfg: Config) -> list[str]:
    ...
    for candidate in candidates:
        labeled.append((" ".join(candidate), candidate))

    labeled.append(("Browse for an editor application...", ["__BROWSE__"]))
    labeled.append(("Use the current macOS default app for normal .py files", ["__DEFAULT_APP__"]))

    labels = [label for label, _ in labeled]
    choice = choose_from_list_macos(
        title="automarimo setup",
        prompt="Choose the editor to use for normal Python files.",
        options=labels,
    )

    if choice is None:
        raise UserFacingError("No editor was selected.")

    selected_cmd = next(cmd for label, cmd in labeled if label == choice)

    if selected_cmd == ["__BROWSE__"]:
        command = choose_application_macos()
        if command is None:
            raise UserFacingError("No editor was selected.")
    ...
    if selected_cmd == ["__DEFAULT_APP__"]:
        save_editor_command(cfg, selected_cmd)
        return selected_cmd

    command = normalize_editor_command(selected_cmd)
    save_editor_command(cfg, command)
    return command


def show_error_dialog_macos(title: str, message: str) -> None:
    script = f'''
display dialog "{applescript_quote(message)}" with title "{applescript_quote(title)}" buttons {{"OK"}} default button "OK"
'''.strip()
...
def launch_marimo_in_macos_terminal(path: Path, cfg: Config, cmd: list[str]) -> int:
    banner = marimo_banner(path).strip()
    ...
    maybe_debug(cfg, f"Launching marimo in macOS Terminal: {apple_script!r}")
...
def run_target(path: Path, cfg: Config, *, dry_run: bool = False) -> int:
    ...
    if is_marimo:
        ...
        if is_macos():
            return launch_marimo_in_macos_terminal(path, cfg, cmd)
